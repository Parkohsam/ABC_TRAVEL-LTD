import packageModel from "../model/package.mode.js";
...
const createPackage = async (req, res) => {
  try {
    ...
    if (!name || !description || !price || !availability || !season) {
      return res
        .status(400)
        .send({ status: false, message: "Please fill in all required fields" });
    }
...
const deletePackage = async (req, res) => {
  try {
    ...
    return res
      .status(200)
      .send({ status: true, message: "Package deleted successfully" });
  } catch (err) {
    return res
      .status(500)
      .send({
